...
async function prepareProduction() {
  console.log('🧹 Limpando dados fictícios e preparando ambiente de produção PASSATURNO...');

  // 1. Limpar ocorrências, histórico, trocas de turno fictícias
  ...
  console.log('✅ Dados fictícios e histórico limpos');
  ...
  console.log(`✅ Matriz da Frota pronta: ${count} equipamentos/repetidoras verificados`);
  console.log('🚀 PASSATURNO pronto para operação real em produção!');
}

prepareProduction()
  .catch((e) => {
    console.error('❌ Erro na preparação de produção:', e);
    process.exit(1);
  })
  .finally(async () => {
